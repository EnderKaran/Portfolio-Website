...
export const Navbar = () => {
    ...
    const renderLink = (link) => {
        if (link.type === 'scroll' && onHomePage) {
            return (
                <ScrollLink to={link.path} spy={true} smooth={true} offset={-100} duration={500} activeClass="active-link" className="text-sm cursor-pointer hover:text-accent transition-colors duration-300">
                    {link.title}
                </ScrollLink>
            );
        }
        ...
        return (
            <RouterLink to={toPath} className="text-sm cursor-pointer hover:text-accent transition-colors duration-300">
                {link.title}
            </RouterLink>
        );
    };
    ...
    return (
        <>
            ...
            <div className="z-50 fixed flex justify-center w-full font-bold"> 
                <div className="border border-emerald-500/40 mt-8 bg-white/80 dark:bg-black/70 backdrop-blur-xl rounded-3xl hidden md:flex items-center justify-center p-2 max-w-fit mx-auto text-slate-800 dark:text-slate-200">
                        ...
                        {navlinks.map((link, index) => (
                            <li key={index}>
                                {link.type === 'scroll' && onHomePage ? (
                                    <ScrollLink to={link.path} spy={true} smooth={true} offset={-70} duration={500} onClick={closeNav} className="text-5xl cursor-pointer hover:text-accent transition-colors">
                                        {link.title}
                                    </ScrollLink>
                                ) : (
                                    <RouterLink to={link.type === 'scroll' ? `/#${link.path}` : link.path} onClick={closeNav} className="text-5xl cursor-pointer hover:text-accent transition-colors">
                                        {link.title}
                                    </RouterLink>
                                )}
                            </li>
